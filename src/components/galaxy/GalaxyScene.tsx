...
function FloatingShip() {
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      ...
      ref.current.rotation.y = baseRotationY + Math.sin(clock.getElapsedTime() * 0.3) * 0.1;
      ref.current.position.y = basePositionY + Math.sin(clock.getElapsedTime()) * 0.05;
    }
  });

  // Set base transforms to preserve initial orientation
  const baseRotationY = Math.PI / 2;
  const basePositionY = 0; // You can tweak this if needed

  return (
    <group ref={ref} position={[0, basePositionY, 0]} rotation={[0, baseRotationY, 0]}>
...
export default function GalaxyScene({ children }: GalaxySceneProps) {
  return (
    <section className="relative w-full h-[500px] bg-gradient-to-b from-[#010218] via-[#050d26] to-[#090e24] overflow-hidden">
      {/* 3D Galaxy Canvas */}
      <Canvas camera={{ position: [0, 1.5, 3], fov: 35 }} className="w-full h-full">
